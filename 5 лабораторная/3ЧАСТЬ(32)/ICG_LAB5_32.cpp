...
public:
    ...
    bool Init()
    {
        ...
        if (!m_pMesh1->LoadMesh("C:/TEXTURE/phoenix_ugv.md2")) {
            return false;
        }

        m_pMesh2 = new Mesh();

        if (!m_pMesh2->LoadMesh("C:/TEXTURE/jeep.obj")) {
            return false;
        }

        m_pMesh3 = new Mesh();

        if (!m_pMesh3->LoadMesh("C:/TEXTURE/hheli.obj")) {
            return false;
        }
...
int main(int argc, char** argv)
{
    ...
    if (!GLUTBackendCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, 32, false, "32")) {
        return 1;
    }
